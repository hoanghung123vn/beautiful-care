    ...
    if (updateDOW) {
      Array.from(this.dow.children).forEach((el, index) => {
        ...
        el.className = this.daysOfWeekDisabled.includes(dow)
          ? 'dow disabled text-center h-6 leading-6 text-sm font-medium text-ink-60 dark:text-ink-40 cursor-not-allowed'
          : 'dow text-center h-6 leading-6 text-sm font-medium text-ink-60 dark:text-ink-40';
      });
    }
  }
  ...
  render() {
    ...
    Array.from(this.grid.children).forEach((el, index) => {
      ...
      if (current < this.first) {
        classList.add('prev', 'text-ink-40', 'dark:text-white');
        classList.remove('text-gray-900');
      } else if (current > this.last) {
        classList.add('next', 'text-ink-40', 'dark:text-white');
        classList.remove('text-gray-900');
      }
